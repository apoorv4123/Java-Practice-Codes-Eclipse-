...
public class FrogJumpUptoKJumps {
	...
	public static void main(String[] args) {
		...
		System.out.println(spaceOptimization(n, 3, height));
	}
	...
	public static int spaceOptimization(int n, int k, int[] ht) {
...
//		ArrayList<Integer> kList = new ArrayList<Integer>();
		...
		for (int j = 1; j < n; j++) {
			...
			for (int i = 1; i <= k; i++) {
				if (j >= i) {
					ans = Integer.min(ans, kList.get(k - i) + Math.abs(ht[j] - ht[j - i]));
				}
			}
			kList.removeFirst();
			kList.addLast(ans);

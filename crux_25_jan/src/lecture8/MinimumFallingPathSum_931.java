...
//				{ 6, -33, 91, -6, -94, 82, 25, -43, -93, -25, -69, 10, -71, -65, 85, 28, -52, 76, 25, 90 },
...
//				{ -67, -14, -20, -85, 42, 36, 56, 9, 11, -66, -59, -55, 5, 64, -29, 77, 47, 44, -33, -77 } };
		...
		int[][] strg = new int[A.length][A[0].length];
	...
	public static int mfpsrec(int[][] arr, int row, int col) {
		...
		// minus 1
		int m1 = mfpsrec(arr, row + 1, col - 1);
		// same col
		int nm = mfpsrec(arr, row + 1, col);
		// plus 1
		int p1 = mfpsrec(arr, row + 1, col + 1);

		return Math.min(m1, Math.min(nm, p1)) + arr[row][col];

	}

	public static int mfpstd(int[][] arr, int row, int col, int[][] strg) {
		...
		// minus 1
		int m1 = mfpstd(arr, row + 1, col - 1, strg);
		// same col
		int nm = mfpstd(arr, row + 1, col, strg);
		// plus 1
		int p1 = mfpstd(arr, row + 1, col + 1, strg);

		int ans = Math.min(m1, Math.min(nm, p1)) + arr[row][col];
